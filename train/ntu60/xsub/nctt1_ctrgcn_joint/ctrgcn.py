...
class TemporalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        att_t = (torch.einsum('ncsv,nctv->ncst', q, k)) / math.sqrt(25.0)  # N*M, V, T, T
        att_t = torch.softmax(att_t, dim=-1)  # N, V, T, T
        out_sa = torch.einsum('ncst,nctv->ncsv', att_t, v)  # N, C, T, V
